...
/proc/convert(mob/user, turf/C)
	...
	for(var/mob/living/carbon/human/H in C.contents)
		if(H != user)
			...
			var/alert = alert(H, "YOU WILL BE SHOWN THE TRUTH. DO YOU RESIST? (Resisting: 1 TRI)", "Frozen Summit", "Yield", "Resist")
			H.anchored = TRUE
			if(alert == "Yield")
...
/obj/item/pactofunity/attack_self(mob/user)
	. = ..()
	var/alert = alert(user, "Rip up the pact of unity?", "Frozen Summit", "RIP", "Cancel")
	if(alert == "RIP")
...
/obj/item/pactofunity/attack(mob/living/M, mob/living/user)
	...
	if(!H.stat)
		...
		if(alert(H, "Sign the pact with your blood?", "Frozen Summit", "Yes", "No") != "Yes")
			return
		if(H.stat)
			return
		if(signed)
