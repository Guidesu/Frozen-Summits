...
/obj/structure/fluff/traveltile/proc/can_go(atom/movable/AM)
	// Cooldown check (moved to top)
	if(AM.recent_travel && world.time < AM.recent_travel + 10 SECONDS)
		return FALSE
...
/obj/structure/fluff/traveltile/Crossed(atom/movable/AM)
	...
	// Check cooldown first (changed position)
	if(!can_go(AM))
		return
