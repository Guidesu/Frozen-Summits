...
/obj/item/rogueweapon/woodstaff/mithral
	force = 20
	force_wielded = 25
	possible_item_intents = list(SPEAR_BASH)
	gripped_intents = list(SPEAR_BASH,/datum/intent/mace/smash)
	name = "mithral pole"
	desc = "Shiny and regal, light enough to beat something without much effort."
	icon_state = "mithralpole"
	icon = 'icons/roguetown/weapons/64.dmi'
	wlength = WLENGTH_LONG
	w_class = WEIGHT_CLASS_BULKY
	slot_flags = ITEM_SLOT_BACK
...
/obj/item/rogueweapon/woodstaff/mithral/getonmobprop(tag)
	. = ..()
	if(tag)
		switch(tag)
			if("gen")
