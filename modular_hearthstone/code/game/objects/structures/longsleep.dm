...
/obj/structure/long_sleep //Shamelessly jury-rigged from the way Fallout13 handles this.
	name = "long-sleep cabins"
	desc = "A door leading into long sleep cabins.\n(Drag your sprite onto this to exit the round!)"
...
/obj/structure/long_sleep/MouseDrop_T(atom/dropping, mob/user)
	...
	user.visible_message("<span class='warning'>[user] [departing_mob == user ? "is trying to leave for the lower residentals!" : "is trying to send [departing_mob] to the lower residentals!"]</span>", "<span class='notice'>You [departing_mob == user ? "are trying to go to the lower decks." : "are trying to send [departing_mob] to the lower decks."]</span>")
	in_use = TRUE
...
/obj/structure/long_sleep2/MouseDrop_T(atom/dropping, mob/user)
	. = ..()
	if(!isliving(user) || user.incapacitated())
		return //No ghosts or incapacitated folk allowed to do this.
	if(!ishuman(dropping))
	...
	if(user.incapacitated() || QDELETED(departing_mob) || (departing_mob != user && departing_mob.client) || get_dist(src, dropping) > 2 || get_dist(src, user) > 2)
		return //Things have changed since the alert happened.
	user.visible_message("<span class='warning'>[user] [departing_mob == user ? "is trying to teleport using the Iodestone!" : "is trying to teleport [departing_mob] using the Iodestone!"]</span>", "<span class='notice'>You [departing_mob == user ? "are trying to go to teleport using the Iodestone." : "are trying to teleport [departing_mob] using the Iodestone."]</span>")
	in_use = TRUE
	if(!do_after(user, 50, target = src))
		in_use = FALSE
		return
	...
	log_admin(dat)
	if(departing_mob.stat == DEAD)
		departing_mob.visible_message("<span class='notice'>[user] teleports the body of [departing_mob] using the Iodestone. They're someone else's problem now.</span>")
	else
